...
## 상태 저장 방식
- MongoDB URI가 주어지면 컬렉션에 `{ project, state, updatedAt }`로 업서트.
- URI가 없거나 실패하면(그리고 `TREE_REQUIRE_MONGO`/`requireMongo`가 아닌 경우) 로컬 `tree/state/<프로젝트>/state.json`을 사용.
